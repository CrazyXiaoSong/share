...
$sv = isset($_GET['sv'])?$_GET['sv']:'';
$page = isset($_GET['page'])?$_GET['page']:1;
if($page<=1){
    $page = 1;
};
$url = "http://www.jq22.com/search?seo=".$sv."&PageNo=".$page;
                    ...
                    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                        ...
                        <form action="search.php" class="navbar-form navbar-right">
                            <div class="form-group">
                                <input type="text" name="sv" class="form-control search_value" placeholder="请输入关键字...">
                            </div>
...
<div class="container">
    ...
    for ($i = 1; $i < count($data[0]); $i++) {
        $url = preg_replace('/http:\/\/www.jq22.com\//',"./jq.php?info=",$data[2][$i]);
    ...
    <nav>
        <ul class="pager">
            <li class="previous <?php echo count($has_prePage[0])<2? 'disabled' : '' ; ?>"><a href="<?php echo count($has_prePage[0])<2? 'javascript:' : '?sv='.$sv.'&page='.($page-1) ; ?>"><span aria-hidden="true">&larr;</span> 上一页</a></li>
            <li class="next <?php echo count($has_nextPage[0])<2? 'disabled' : '' ;  ?>"><a href="<?php echo count($has_nextPage[0])<2? 'javascript:' : '?sv='.$sv.'&page='.($page+1) ;  ?>">下一页 <span aria-hidden="true">&rarr;</span></a></li>
        </ul>
    </nav>
</div>
</body>
